...
#define BATCH_SIZE 32 // Tamanho do lote para processamento paralelo
...
bool ctx_check_hash(ctx_t *ctx, const h160_t h) {
  if (ctx->to_find_hashes == NULL) return blf_has(&ctx->blf, h);
...
void check_found_add(ctx_t *ctx, const fe *privs, const pe *points, size_t batch_size) {
  h160_t hs33[HASH_BATCH_SIZE];
  h160_t hs65[HASH_BATCH_SIZE];
  for (size_t i = 0; i < batch_size; i += HASH_BATCH_SIZE) {
    size_t current_batch = MIN(HASH_BATCH_SIZE, batch_size - i);
    if (ctx->check_addr33) addr33_batch(hs33, points + i, current_batch);
    if (ctx->check_addr65) addr65_batch(hs65, points + i, current_batch);
    for (size_t j = 0; j < current_batch; ++j) {
      if (ctx->check_addr33) check_hash(ctx, true, hs33[j], privs[i + j], 0, 0);
      if (ctx->check_addr65) check_hash(ctx, false, hs65[j], privs[i + j], 0, 0);
    }
  }
  if (!ctx->use_endo) return;
  size_t esize = batch_size * 5;
  pe endos[esize];
  ...
  for (size_t k = 0; k < batch_size; ++k) {
    ...
    bool is_full = (idx + 5) % esize == 0 || k == batch_size - 1;
    if (!is_full) continue;
    for (size_t i = 0; i < esize; i += HASH_BATCH_SIZE) {
      size_t current_batch = MIN(HASH_BATCH_SIZE, esize - i);
      if (ctx->check_addr33) addr33_batch(hs33, endos + i, current_batch);
      if (ctx->check_addr65) addr65_batch(hs65, endos + i, current_batch);
      for (size_t j = 0; j < current_batch; ++j) {
        if (ctx->check_addr33) check_hash(ctx, true, hs33[j], privs[ci / 5], 0, (ci % 5) + 1);
        if (ctx->check_addr65) check_hash(ctx, false, hs65[j], privs[ci / 5], 0, (ci % 5) + 1);
        ci += 1;
      }
    }
  }
  assert(ci == batch_size * 5);
}

void batch_add(ctx_t *ctx, const fe start_pk, const size_t iterations) {
  fe privs[BATCH_SIZE];
  pe points[BATCH_SIZE];
  fe ck;
  fe_clone(ck, start_pk);
  for (size_t counter = 0; counter < iterations; counter += BATCH_SIZE) {
    size_t current_batch = MIN(BATCH_SIZE, iterations - counter);
    for (size_t i = 0; i < current_batch; ++i) {
      fe_modn_add_stride(privs[i], ck, ctx->stride_k, i);
    }
    multi_ec_mul(points, privs, current_batch);
    ec_jacobi_grprdc_batch_avx2(points, current_batch);
    check_found_add(ctx, privs, points, current_batch);
    fe_modn_add_stride(ck, ck, ctx->stride_k, current_batch);
  }
}
...
void *cmd_mul_worker(void *arg) {
  ...
  while (true) {
    ...
    multi_ec_mul(cp, pk, job->count);
    ec_jacobi_grprdc_batch_avx2(cp, job->count);
